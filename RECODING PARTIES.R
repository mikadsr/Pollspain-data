  ...
  parties_data <- parties_data %>%
    mutate(
      name_candidacies = case_when(
        ...
        str_detect(name_candidacies, "UNION DEL PUEBLO NAVARRO EN COALICION CON EL PARTIDO|PARTIDO POPULAR CENTRISTAS DE GALICIA|UNION DEL PUEBLO NAVARRO CON PARTIDO POPULAR|UNIÓN DEL PUEBLO NAVARRO EN COALICIÓN CON EL PARTI|^PARTIDO POPULAR-|PARTIT POP") ~ "PARTIDO POPULAR",
        str_detect(name_candidacies, "COALICION CANARIA-NUEVA CANARIAS|NUEVA CANARIAS-COALICION CANARIA") ~ "COALICION CANARIA-NUEVA CANARIAS",
        str_detect(name_candidacies, "CIUTADANS-PARTIDO DE LA CIUDADANIA|CIUDADANOS-PARTIDO DE LA CIUDADANIA") ~ "CIUDADANOS",
        str_detect(name_candidacies, "MES COMPROMIS") ~ "MES COMPROMIS",
        str_detect(name_candidacies, "EN COMUN-UNIDAS PODEMOS|UNIDAS PODEMOS|UNIDES PODEM|ELKARREKIN PODEMOS-UNIDAS PODEMOS|UNITS PODEM MES|COMPROMIS-PODEMOS-EUPV: A LA VALENCIANA|EN MAREA|PODEMOS") ~ "UNIDAS PODEMOS",
        str_detect(name_candidacies, "PART SOCIALISTA OBRERO ESPANOL DE ANDALUCIA|
PART SOCIALISTA OBRERO ESPANOL DE ANDALUCIA|SOCIALISTES|PARTIT DELS SOCIALISTES|PARTIT SOCIALISTA OBRER ESPANYOL|^PARTIDO DOS SOCIALISTAS|^PARTIDO SOCIALISTA") ~ "PARTIDO SOCIALISTA OBRERO ESPANOL",
        str_detect(name_candidacies, "CANDIDATURA DUNITAT POPULAR-PER LA RUPTURA|CUP-PR") ~ "CANDIDATURA DUNITAT POPULAR-PER LA RUPTURA",
        str_detect(name_candidacies, "UNION DEL PUEBLO NAVARRO-PARTIDO POPULAR|UNION DEL PUEBLO NAVARRO EN COALICION CON EL PARTI") ~ "UNION DEL PUEBLO NAVARRO EN COALICION CON EL PARTIDO",
        str_detect(name_candidacies, "EHBILDU|EH BILDU") ~ "EH-BILDU",
        str_detect(name_candidacies, "!TERUEL EXI") ~ "TERUEL EXISTE",
        str_detect(name_candidacies, "ALIANZA POP-PDEMOCPOPULAR-PDEMOCLIBERAL-UCD|^ALIANZA POPULAR|^UPN-AP") ~ "ALIANZA POPULAR",
        str_detect(name_candidacies, "ARA|PV") ~ "ARA REPUBLIQUES",
        str_detect(name_candidacies, "BNG-NOS|BNG-NOS|NOS") ~ "BLOQUE NACIONALISTA GALEGO",
        ...
        str_detect(name_candidacies, "ERC-CATSI|ERC") ~ "ESQUERRA REPUBLICANA",
      ...
      abbrev_candidacies = case_when(
        ...
        str_detect(name_candidacies, "^PARTIDO SOCIALISTA|SOCIALISTES|PARTIT DELS SOCIALISTES|PARTIT SOCIALISTA OBRER ESPANYOL|PARTIDO DOS SOCIALISTAS DE GALICIA-PARTIDO SOCIALI|PARTIDO SOCIALISTA DE EUSKADI-EUSKADIKO EZKERRA \\(P\\)") ~ "PSOE",
        ...
        str_detect(name_candidacies, "AP") ~ "AP",
        str_detect(name_candidacies, "ARA|PV") ~ "ARA-PV",
        str_detect(name_candidacies, "BNG-NOS|BNG-NOS|NOS") ~ "BNG",
        ...
        str_detect(name_candidacies, "ERC-CATSI|ERC") ~ "ERC",
