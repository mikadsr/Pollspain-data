...
historical_raw_candidacies_poll |>
  ...
  map(function(y) { map(y, function(x) {
    write_csv(x, file = glue("./data/csv/candidacies_pollstation/raw_candidacies_poll_{unique(x$type_elec)}_{year(unique(x$date_elec))}_{month(unique(x$date_elec))}.csv"))
  })})
...
print(congress_poll_test)

# Test for candidacies poll data in senate elections in June 1986
senate_poll_test <- import_raw_candidacies_poll_file("senate", 1986, 6)
print(senate_poll_test)
