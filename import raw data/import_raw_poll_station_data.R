...
# Function to import raw poll station data files from MIR
import_poll_stations_MIR_files <- function(type_elec, year, month,
  ...
  dat_file_name <- glue("09{cod_elec}{str_sub(year, start = 3, end = 4)}{char_month}.DAT")
...
historical_raw_poll_station_data |>
  ...
  map(function(y) { map(y, function(x) {
    write_csv(x, file = glue("./data/csv/pollstation/raw_poll_station_data_{unique(x$type_elec)}_{year(unique(x$date_elec))}_{month(unique(x$date_elec))}.csv"))
  })})
...
# Test for poll station data in congress elections in June 1986
congress_poll_station_test <- import_poll_stations_MIR_files("congress", 1986, 6)
print(congress_poll_station_test)
